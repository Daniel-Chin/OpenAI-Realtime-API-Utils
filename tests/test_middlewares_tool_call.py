...
if LOG_STDOUT:
    ...
    sys.stdout = TeeOutput('./tests/logs/test_middlewares_tool_call.log', sys.stdout)
    # sys.stderr = TeeOutput('./tests/logs/test_middlewares_tool_call.log', sys.stderr)
    ...
    with py_audio_context() as pa:
        ...
        async with a_r.connect(
            ...
            with (
                ...
                middlewares.ToolCallOnSpeechEnd(
                    [functools.partial(
                        print, 'Tool call emitted on speech end:', 
                    )], audio_player.register_on_speech_end_handler, 
                ).context() as tool_call_on_speech_end,
                hook_handlers(
                    connection, 
                    # All middlewares are optional.  
                    server_event_handlers = [
                        ...
                        *iap_server_handlers,   # views e.g. response.output_audio.delta
                        ...
                        tool_call_on_speech_end.server_event_handler, # views tool calls
                ...
                await send(tp_rt.SessionUpdateEventParam(
                    ...
                    session=tp_rt.RealtimeSessionCreateRequestParam(
                        ...
                        tools=[
                            tp_rt.RealtimeFunctionToolParam(
                                type='function', 
                                name='turn_lamp_on',
                ...
                await send(tp_rt.ConversationItemCreateEventParam(
                    ...
                    item=tp_rt.RealtimeConversationItemUserMessageParam(
                        ...
                        content=[tp_rt.realtime_conversation_item_user_message_param.Content(
                            type='input_text',
                            text='The room is too dark. Turn on the lamp as you explain what you are doing, briefly.',
                        )],
                    ),
                ))
